...
`cmd_accinfo`, `cmd_delreg`, `cmd_disconnect`, `cmd_gag`, `cmd_myinf`,
`cmd_myip`, `cmd_redirect`, `cmd_reg`, `cmd_setpass`, `cmd_sslinfo`,
`cmd_upgrade`, `cmd_userinfo`, `cmd_usersearch`, `etc_backup`,
`etc_blocklist`, `etc_forcetlstransfer`, `etc_lockdown`, `etc_whitelist`,
`hub_runtime`, `usr_hide_share` - export **only** such
