...
| `meta` | table or nil | Optional metadata - `{plugin=, description=, request_schema=, response_schema=, audit_redact_body=, min_level=}`. `plugin` is the source plugin name, for `/v1/endpoints` + `/v1/plugins` attribution. Surfaced via `/v1/endpoints` (except `audit_redact_body`, which is router-internal). Used by WebUI for form rendering. `audit_redact_body = true` opts the route into §8 audit-body redaction (used by password endpoints). `min_level` (integer, #726) is the minimum operator level the WebUI should let attempt the route - set it to the command's own invocation floor on a route whose handler enforces a per-operator ceiling/hierarchy, so the WebUI can gate per operator instead of a flat admin threshold; surfaced via `/v1/endpoints`, omit it on scope-only admin routes |
...
- `min_level` (integer, #726) is present on the moderation / reg "verb"
  routes whose hub handler already enforces a per-operator `cmd_*_permission`
  ceiling or the #718 hierarchy (ban/unban, gag/ungag, redirect, disconnect,
  reg/delreg/setpass/upgrade/nickchange). It is the minimum operator level the
  WebUI should let attempt the route - equal to the ADC command's own
  invocation floor, so it tracks the operator's `cfg.levels`. It is **absent**
  on scope-only admin routes (config/plugins/hub-control), where a consumer
  falls back to its own admin threshold (`webui_admin_min_level`). It is a
  UX/authorisation hint for the BFF; the hub remains the authoritative gate.
...
| GET | `/v1/users/{sid}` | read | full INF + session metadata (same per-user object as `/v1/users`, incl. `firstnick` [^http-users-filter]) |
...
| GET | `/v1/webui-policy` | read | WebUI access thresholds (login floor + admin fallback) [^http-webui-policy] |
...
[^http-config-2]: #262. Body `{value: <any JSON type>}`, required. Path variable `{key}` is the bare cfg key name. Mutates via `cfg.set` (validator check + atomic write to `cfg.tbl`). The on-disk write is **comment/layout-preserving** ([#644](https://github.com/luadch-ng/luadch-ng/issues/644)): `cfg.set` rewrites only the changed key's value span in place - keeping the operator's inline comments, key order and bare-key `key = value` formatting - and falls back to the full canonical re-serialisation only when that surgical rewrite cannot be verified (it re-parses its own output and requires it to match the in-memory config before writing, so the fallback never risks a corrupt file). A JSON object value keyed by level number (a level-keyed map like `cmd_ban_permission`) arrives with **string** keys (`{"0":0}`); the cfg validators for these maps require **integer** keys, so the handler coerces an all-integer-string-keyed table to integer keys before validating (a table with any non-integer key is left as-is, so a legitimately string-keyed value still surfaces its own error) - [#630](https://github.com/luadch-ng/luadch-ng/issues/630). Response carries `apply_status`, classified in `core/http_router.lua` from a static core-key table **plus a dynamic per-plugin set** (keys a plugin declares reload-required at load via `cfg.mark_reload_required`, consulted after the static table - `etc_geoip` is the first consumer, marking all 18 of its `etc_geoip_*` keys, [#707](https://github.com/luadch-ng/luadch-ng/issues/707)): `live` (effect on `cfg.set` - default for the ~200 keys not in either bucket or the plugin set); `reload_required` (needs `POST /v1/reload` to apply; currently `scripts`, `language`, the path-pointer keys `user_path` / `script_path` / `scripts_cfg_path` / `scripts_lang_path` / `core_lang_path`, and the rate-limiter tuning keys - every `ratelimit_*` key plus the HTTP-API knobs `http_api_rate_read` / `http_api_rate_admin` / `http_api_burst` / `http_api_authfail_prefix_rate` / `http_api_authfail_prefix_burst` / `http_api_authverify_rate` / `http_api_authverify_burst`, which re-read their cfg and reset their token buckets on reload since [#648](https://github.com/luadch-ng/luadch-ng/issues/648)); `restart_required` (needs full hub restart; currently `tcp_ports`, `ssl_ports`, `tcp_ports_ipv6`, `ssl_ports_ipv6`, `http_port`, `hub_listen`, `master_key_path`, `log_path`, `ssl_params`, `use_ssl`). Returns 403 `E_FORBIDDEN` if `{key}` is on the denylist (sensitive credentials rotate via direct cfg.tbl edit + restart); 404 `E_NOT_FOUND` if the key is not registered in `cfg_defaults.lua`; 400 `E_BAD_INPUT` if body is missing the `value` field OR the validator rejected the value (the validator's err_msg is surfaced in the response). The change is NOT auto-reload-triggered - the operator chains `POST /v1/reload` for reload-required keys, or restarts the hub for restart-required keys. Per-key validator schemas are NOT exposed (the validator closures aren't designed to be introspectable); deferred to a future `GET /v1/config/{key}/schema` endpoint if a real need emerges.

[^http-webui-policy]: #726. Returns 200 with `data: {min_level, admin_min_level}` - the two global WebUI access thresholds (cfg keys `webui_min_level` / `webui_admin_min_level`, defaulting to 60 / 80 = OPERATOR / ADMIN when unset), in the hub's own `cfg.levels` numbering. A small curated read-model over cfg so the WebUI BFF depends on a stable contract instead of scraping the full `/v1/config` snapshot for two values. `min_level` is the login floor (who may sign in to the WebUI); `admin_min_level` is the required level for admin-scoped routes that advertise no per-endpoint `min_level` (config, plugins, hub control). Per-route floors ride on `GET /v1/endpoints` (`min_level`) and override `admin_min_level` per endpoint. The hub does not act on these keys - it only advertises them; the WebUI BFF is the authorization gate.
...
|---|---|---|---|
| DELETE | `/v1/users/{sid}` | admin | `cmd_disconnect` [^http-disconnect] |
...
[^http-ban-3]: Body `{target_type: "nick"|"cid"|"ip"|"sid" required, target: string required (max 64), duration_minutes: integer optional (1..525600 = 1 year cap, schema-enforced; falls back to cfg `cmd_ban_default_time` if omitted), permanent: boolean optional (default false), reason: string optional (max 256)}`. **`permanent: true` bans forever** (= ADC `+ban ... permanent`, #444): it ignores `duration_minutes`, stores a `permanent` marker with `ban_seconds 0`, and the response omits `duration_minutes` + `expires_at`. Returns 200 with `data: {action:"ban", id, target_type, target, target_nick?, duration_minutes?, permanent, reason, by, expires_at?}` per §7.1.1 (`duration_minutes` and `expires_at` present only for a time-limited ban). `target_nick` is present only when the target was resolved to an online user (sid path always; nick/cid/ip if currently online). Resolution rules: `sid` is strict-online-only (404 if not online); `nick` falls back to `hub.getregusers()` for offline registered nicks (404 if neither online nor registered); `cid`/`ip` are blind-add (no offline lookup - matches the ADC `+ban` behaviour). Persisted entries get `by_level = 100` so they cannot be lifted by lower-level operators via the ADC `+unban` cmd (only level >= ban.by_level can lift). Online targets are kicked with `ISTA 232 ... TL<bantime>` (time-limited) or `ISTA 231 ... TL-1` (permanent), matching the ADC path. **#177 C: the banned user's kick message - AND the message they see on a later reconnect** (via the persisted `http` origin flag) **- name the hubbot** (never the operator nick or a token label), while the stored `by_nick`, the response `by`, the opchat report, and the audit trail record the real operator (`req.actor` / X-Actor, falling back to the token label then `"http-api"`). ADC `+ban` entries are unaffected (their reconnect message keeps the OP nick). `reason` and the actor strings are control-byte sanitised (`util.strip_control_bytes`) before they hit the bans table or the frames. **Permission ceiling (#708):** when the actor resolves to a known operator level - the trusted BFF path asserts the authenticated operator's nick in `X-Actor`, which `util_http.actor_level` maps to its registered level - the ban is refused with **403 E_FORBIDDEN** (`"target level exceeds your ban permission ceiling"`) if the target's level exceeds `cmd_ban_permission[operator_level]`, the same hierarchy guard the ADC `+ban` enforces. A direct token call that sends no `X-Actor` (no resolvable operator level) keeps the scope-only behaviour, and `cid`/`ip` blind targets have no resolvable target level and are not ceiling-checked (matching the ADC path, whose `cid`/`ip` branches are likewise unchecked). Enforced BEFORE the ban is persisted, so a denied attempt mutates nothing. The ceiling actor is resolved from `X-Actor` treated as the operator's **base registered nick** - `util_http.actor_level` looks it up in `hub.getregusers()` verbatim (from the un-mangled `req.actor_raw`, so a nick containing `=` still matches). A caller must therefore send the operator's base registered nick, NOT a `usr_nick_prefix`-decorated display nick, or the operator does not resolve and the ceiling is skipped for that request (fail-open, but a raw-token caller is already fully privileged - the ceiling is a cooperative hierarchy control, not the auth gate).

[^http-ban-4]: `{id}` is the 1-based index from GET /v1/bans. Returns 200 with `data: {action:"unban", id, removed:{nick, cid, ip, reason, by_nick}, by}` so the operator's audit / undo flow has the snapshot of what was lifted. The response `id` echoes the requested index (which no longer exists post-removal, and may even point to a different victim now if a concurrent mutation reindexed the array); for any audit or undo workflow the stable identifiers are the `removed.nick` / `removed.cid` / `removed.ip` fields, NOT the response `id`. Returns **404 E_NOT_FOUND** if there is no entry at that index, **400 E_BAD_INPUT** if `{id}` is not a positive integer. The index race window (between GET and DELETE another mutation reindexes the array) is documented but not papered over: the DELETE always operates on `bans[id]` as-of the request, so a concurrent removal can cause a misaligned delete. Operator tooling MUST refresh the list between deletes to be safe; for batch deletes, sort the targets by descending id (so earlier removals don't shift later indices). The `+unban` ADC cmd uses nick/cid/ip lookups instead and is unaffected. **Permission ceiling (#708):** when `X-Actor` resolves to a known operator level (`util_http.actor_level`, from the un-mangled `req.actor_raw` = the operator's base registered nick), the unban is refused with **403 E_FORBIDDEN** if the ban entry's recorded `by_level` exceeds `cmd_unban_permission[operator_level]` - the same hierarchy guard the ADC `+unban` enforces - checked after the 404 and BEFORE the ban is removed. HTTP-created bans store `by_level = 100`, so only a ceiling >= 100 operator lifts them. A call with no resolvable operator level (no `X-Actor`) keeps the scope-only behaviour.
...
| GET | `/v1/stats/history?range=24h\|7d\|30d` | read | `etc_stats_history` [^http-stats-history-1] |
...
[^http-records-1]: Returns 200 with `data: {hub_share: {total_bytes, recorded_at}, max_users: {count, recorded_at}, top_sharer: {nick, share_bytes}, hub_files: {count, recorded_at}, top_file_sharer: {nick, file_count}}`. Raw byte / file counts (no unit formatting - clients decide display units); ADC `+records show` builds a `MB / GB / TB / PB` string via `shareoptimize`, the HTTP path does NOT (consistent with `/v1/stats` raw-bytes convention). `recorded_at` is `YYYY-MM-DD / HH:MM:SS` (hub local time - matches `cmd_reg`'s persistence format, not ISO 8601); collapses to `""` when both halves of the persisted record are missing (never-sampled hub). On a fresh hub before any sample fires, all counters default to `0` - `hub_share.total_bytes`, `max_users.count`, `top_sharer.share_bytes`, `hub_files.count`, `top_file_sharer.file_count` - with `top_sharer.nick` / `top_file_sharer.nick` defaulting to `"none"`, so a value of `0` unambiguously means "no record yet". `hub_files` (peak total shared-file count, the SF partner to `hub_share`) and `top_file_sharer` (biggest single-user file count, the partner to `top_sharer`) were added in #647; both are silent (no chat broadcast on a new peak, unlike the three legacy records). The record store migrated in #647 from a positional 8-slot table to a named-key table (`records.hub_share.bytes`, ...); the migration on load preserves every existing value, so a 3.1.x operator keeps their records across the 3.1 -> 3.2 upgrade. (#618 normalised the `hub_share` seed from a vestigial `1` to `0`; the `> records.hub_share.bytes` max-tracker in `hubshare()` increments identically from a `0` seed since no real hub totals exactly 1 byte. Hubs recorded before the fix keep their persisted value until the next `+records reset`.)
...
[^http-stats-history-1]: #665 (hub side of [webui#183](https://github.com/luadch-ng/webui/issues/183)). Returns 200 with `data: {range, interval_sec, points: [{t, count}, ...]}` - the online-**human** user count (same figure as `/v1/stats` `online_count`) sampled every 60 seconds and consolidated RRD-style into the tier matching `range` (each bucket keeps the peak of its samples), oldest -> newest. `t` is Unix epoch seconds (bucket start, aligned to the tier's slot); `count` is the **max** online count observed in that bucket (peak concurrent). `interval_sec` is the tier's slot width. Ranges: `24h` (10-min slots, up to 144 points), `7d` (1-h slots, up to 168), `30d` (6-h slots, up to 120); an unknown or absent `range` falls back to `24h`. The three tiers are separate round-robin buffers persisted to `scripts/data/etc_stats_history.tbl` (survives `+reload` and restart), so no server-side downsampling is done - the endpoint just returns the requested tier. No backfill: a freshly-enabled plugin (or a fresh hub) returns few or `[]` points and accretes history as it runs. Provided by the bundled `etc_stats_history` plugin, which **ships enabled** (fresh installs collect out of the box); when the plugin is disabled the route is not registered and the router returns 404 `E_NOT_FOUND`. Read scope (matches `/v1/stats`).
